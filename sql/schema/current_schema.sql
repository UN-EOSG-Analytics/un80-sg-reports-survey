...
create index if not exists idx_magic_tokens_cleanup on sg_reports_survey.magic_tokens (expires_at)
where (used_at IS NULL);
create table if not exists sg_reports_survey.reports (
    id serial primary key,
    record_number text,
    symbol text not null constraint unique_symbol unique,
    symbol_split text [],
...
create index if not exists idx_reports_text_search on sg_reports_survey.reports using gin (
    to_tsvector('english'::regconfig, COALESCE(text, ''::text))
);
create index if not exists idx_reports_embedding on sg_reports_survey.reports using hnsw (embedding public.vector_cosine_ops);
create index if not exists idx_reports_based_on_resolution_symbols on sg_reports_survey.reports using gin (based_on_resolution_symbols);
create index if not exists idx_reports_document_category on sg_reports_survey.reports (document_category);
create table if not exists sg_reports_survey.allowed_domains (
...
create table if not exists sg_reports_survey.report_entity_suggestions (
    ...
    entity text not null references ? ? ? (),
    source text not null constraint report_entity_suggestions_source_check check (
        source = ANY (ARRAY ['dgacm'::text, 'dri'::text, 'ai'::text])
    ),
    confidence_score numeric(4, 3) constraint report_entity_suggestions_confidence_score_check check (
...
create table if not exists sg_reports_survey.report_entity_confirmations (
    ...
    proper_title text not null,
    entity text not null references ? ? ? (),
    confirmed_by_user_id uuid not null references sg_reports_survey.users,
    confirmed_at timestamp with time zone default now(),
    notes text,
    role text default 'lead'::text not null constraint report_entity_confirmations_role_check check (
        role = ANY (ARRAY ['lead'::text, 'contributing'::text])
    ),
...
create table if not exists sg_reports_survey.documents (
    id serial primary key,
    record_number text,
    symbol text not null constraint unique_document_symbol unique,
    symbol_split text [],
    symbol_split_n integer,
    document_category text,
    session_or_year text,
    ...
    updated_at timestamp with time zone default now(),
    word_count integer,
    data_source text default 'library'::text not null constraint documents_data_source_check check (
        data_source = ANY (ARRAY ['library'::text, 'manual'::text])
    ),
    created_by_user_id uuid references sg_reports_survey.users
);
...
create index if not exists idx_documents_text_search on sg_reports_survey.documents using gin (
    to_tsvector('english'::regconfig, COALESCE(text, ''::text))
);
create index if not exists idx_documents_embedding on sg_reports_survey.documents using hnsw (embedding public.vector_cosine_ops);
create index if not exists idx_documents_data_source on sg_reports_survey.documents (data_source);
create table if not exists sg_reports_survey.resolution_mandates (
    id serial primary key,
...
create unique index if not exists idx_resolution_mandates_unique on sg_reports_survey.resolution_mandates (
    resolution_symbol,
    md5(COALESCE(verbatim_paragraph, ''::text))
);
create table if not exists sg_reports_survey.survey_responses (
    ...
    status text not null constraint survey_responses_status_check check (
        status = ANY (
            ARRAY ['continue'::text, 'merge'::text, 'discontinue'::text]
        )
    ),
    frequency text constraint survey_responses_frequency_check check (
        (frequency IS NULL)
        OR (
            frequency = ANY (
                ARRAY ['multiple-per-year'::text, 'annual'::text, 'biennial'::text, 'triennial'::text, 'quadrennial'::text, 'one-time'::text]
            )
        )
    ),
    format text constraint survey_responses_format_check check (
        (format IS NULL)
        OR (
            format = ANY (
                ARRAY ['shorter'::text, 'oral'::text, 'dashboard'::text, 'other'::text, 'no-change'::text]
            )
        )
    ),
    format_other text,
    ...
    responded_by_user_id uuid not null references sg_reports_survey.users,
    constraint survey_responses_unique_per_user_per_report_body unique (
        proper_title,
        normalized_body,
...
update on sg_reports_survey.survey_responses for each row execute procedure sg_reports_survey.update_updated_at_column();
...
create index if not exists idx_report_frequencies_frequency on sg_reports_survey.report_frequencies (calculated_frequency);
create index if not exists idx_report_frequencies_body on sg_reports_survey.report_frequencies (normalized_body);
create table if not exists sg_reports_survey.report_frequency_confirmations (
    id serial primary key,
    proper_title text not null,
    normalized_body text default ''::text not null,
    frequency text not null constraint report_frequency_confirmations_frequency_check check (
        frequency = ANY (
            ARRAY ['multiple-per-year'::text, 'annual'::text, 'biennial'::text, 'triennial'::text, 'quadrennial'::text, 'quinquennial'::text, 'one-time'::text, 'other'::text]
        )
    ),
    confirmed_by_user_id uuid not null references sg_reports_survey.users,
    confirmed_at timestamp with time zone default now(),
...
create index if not exists idx_freq_confirmations_frequency on sg_reports_survey.report_frequency_confirmations (frequency);
create index if not exists idx_freq_confirmations_user on sg_reports_survey.report_frequency_confirmations (confirmed_by_user_id);
create index if not exists idx_freq_confirmations_body on sg_reports_survey.report_frequency_confirmations (normalized_body);
create table if not exists sg_reports_survey.ai_chat_logs (
    id bigserial primary key,
    session_id text not null,
    user_id uuid references sg_reports_survey.users,
...
create table if not exists sg_reports_survey.admin_emails (email text not null primary key);
comment on table sg_reports_survey.admin_emails is 'Whitelist of email addresses that receive admin privileges. ';